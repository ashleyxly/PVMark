...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run UPV/unforgeable watermark baseline on current C4 prompts.")
    append_common_generation_args(parser)
    parser.set_defaults(
        output_dir=f"{DEFAULT_RESULTS_ROOT}/unforgeable/opt1.3b_c4_num100",
    )
...
def score_ids(watermark: Any, token_ids: torch.Tensor, threshold: float) -> tuple[Optional[float], bool]:
    if token_ids.ndim > 1:
...
def main() -> None:
    ...
    for row in tqdm(records, desc="UPV generation"):
        ...
        try:
            ...
            with WallTimer() as det_plain_timer:
                plain_score, plain_pred = score_ids(watermark, plain_new_ids, args.z_threshold)
            with WallTimer() as det_wm_timer:
                wm_score, wm_pred = score_ids(watermark, wm_new_ids, args.z_threshold)
            ...
            if torch.cuda.is_available():
                try:
                    ...
                    pass
            failure = make_failure(sample_id, "unforgeable_generation", exc)
