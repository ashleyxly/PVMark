...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run selected MarkLLM attacks for baseline outputs.")
    parser.add_argument("--input", required=True, help="Generation JSON with records.")
    parser.add_argument("--output", required=True)
    parser.add_argument(
        "--attacks",
        default="synonym_substitution,context_aware_synonym_substitution",
        help="Comma-separated subset: synonym_substitution,context_aware_synonym_substitution,word_deletion",
    )
...
def main() -> None:
    args = parse_args()
    # Offset by shard so two shards do not make identical random choices.
    set_seed(args.seed + args.shard_index)
    selected = {name.strip() for name in args.attacks.split(",") if name.strip()}
    allowed = {
        "word_deletion",
    ...
    if unknown:
        raise SystemExit(f"unknown attacks: {sorted(unknown)}")

    WordDeletion, SynonymSubstitution, ContextAwareSynonymSubstitution = import_markllm()
    device = "cuda" if torch.cuda.is_available() else "cpu"
    tokenizer = None
    model = None
    if "context_aware_synonym_substitution" in selected:
        tokenizer = BertTokenizer.from_pretrained(args.bert_path)
        model = BertForMaskedLM.from_pretrained(args.bert_path).to(device)
        model.eval()

    attack_objects: dict[str, Any] = {}
    if "word_deletion" in selected:
        attack_objects["word_deletion"] = WordDeletion(ratio=args.word_deletion_ratio)
    if "synonym_substitution" in selected:
        attack_objects["synonym_substitution"] = SynonymSubstitution(ratio=args.synonym_ratio)
    if "context_aware_synonym_substitution" in selected:
        attack_objects["context_aware_synonym_substitution"] = ContextAwareSynonymSubstitution(
    ...
    for record in records:
        ...
        errors: dict[str, str] = {}
        for name, attack in attack_objects.items():
            with Timer() as t:
                try:
    ...
    write_json(
        output_path,
        {
            "metadata": {
                ...
                "selected_attacks": sorted(selected),
