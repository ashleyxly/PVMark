...
use halo2_merkle_tree::poseidon::spec_width_4::{POSEIDON_WIDTH, POSEIDON_RATE};
use halo2_merkle_tree::poseidon::spec_width_4::PoseidonSpec;
...
// /mnt/disk2/username/kzg-halo2-merkle-tree/test_result/test_verify_detect_text_watermark/input.txt
   ...
   #[arg(short, long, default_value = "/mnt/disk2/username/lm-watermarking/test_result/halo2_circuit_public_inputs_three_to_one.txt")]
   input_file_path: String,

   /// The path of output file including counting results
   #[arg(short, long, default_value = "/mnt/disk2/username/kzg-halo2-merkle-tree/test_result/test_verify_detect_text_watermark/output_three_to_one.txt")]
...
//    scale: i32,
   ...
   #[arg(short, long, default_value_t = 20240425)]
   secret_key: usize,
...
impl Circuit<Fp> for DetectTextWatermarkCircuit {
    ...
    fn configure(meta: &mut ConstraintSystem<Fp>) -> DetectTextWatermarkConfig {
        ...
        DetectTextWatermarkConfig {
            ...
            /*poseidon_3 */
            poseidon_config: PoseidonChip::<PoseidonSpec, WIDTH, RATE, RATE>::configure(meta, [advice_1, advice_2, advice_3, advice_4], advice_many[0], rc_a, rc_b, instance_1),
    ...
    fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<Fp>) -> Result<(), halo2_proofs::plonk::Error> {
        ...
        let mut hash_input2 = [Fp::from(self.secret_key), Fp::from(self.last_prompt_index as u64), Fp::from(self.text_token_index_list[0] as u64)];
        ...
        // let mut hash_input1_cell: [AssignedCell<Fr, Fr>; 3] = poseidon_chip.load_private_inputs(layouter.namespace(|| "Load Poseidon Hash Function Inputs"), [Value::known(hash_input1[0]), Value::known(hash_input1[1])])?;
        // let mut seed_cell = poseidon_chip.hash(layouter.namespace(|| "Poseidon Hash -- Compute Random Seed"), &hash_input1_cell)?;

        let mut hash_input2_cell = poseidon_chip.load_private_inputs(layouter.namespace(|| "Load Poseidon Hash Function Inputs"), [Value::known(hash_input2[0]), Value::known(hash_input2[1]), Value::known(hash_input2[2])])?;
        ...
        let mut last_prompt_index_cell = hash_input2_cell[1].clone();
        token_index_cell_vec.push(hash_input2_cell[2].clone());
        ...
        for round in 0..self.text_token_value_list_length-1 {
            ...
            hash_input2 = [Fp::from(self.secret_key), Fp::from(self.text_token_index_list[round] as u64), Fp::from(self.text_token_index_list[round+1] as u64)];
            ...
            hash_input2_cell = poseidon_chip.load_private_inputs(layouter.namespace(|| "Load Poseidon Hash Function Inputs"), [Value::known(hash_input2[0]), Value::known(hash_input2[1]), Value::known(hash_input2[2])])?;
...
fn generate_test_case(vocabulary_size: usize) -> (Fp, usize, Vec<usize>, usize, Vec<Fp>, usize) {
    ...
    for _ in 0..text_token_value_list_length {
        ...
        let mut temp3 = Fp::from(rng.gen_range(0..1000) as u64);
    ...
    let mut hash_input2 = [Fp::from(serect_key), Fp::from(last_prompt_index as u64), Fp::from(real_index as u64)];
    let mut seed2 = halo2_gadgets::poseidon::primitives::Hash::<_, PoseidonSpec, ConstantLength<RATE>, WIDTH, RATE>::init()
    .hash(hash_input2);
    ...
    for i in 0..text_token_value_list_length-1 {
        ...
        hash_input2 = [Fp::from(serect_key), Fp::from(text_token_index_list[i] as u64), Fp::from(real_index as u64)];
        seed2 = halo2_gadgets::poseidon::primitives::Hash::<_, PoseidonSpec, ConstantLength<RATE>, WIDTH, RATE>::init()
        .hash(hash_input2);
        (seed2, _) = dec_num_fp(seed2, mod_index, DEC_NUM);
        // seed2 = sub_fp(seed2.clone());
...
fn get_public_inputs_from_file(filename: &str, max_token: usize) -> Result<(Vec<String>, Vec<usize>), std::io::Error> {
    ...
    for (index, line) in reader.lines().enumerate() {
        ...
        if index < max_token {
            if let Ok(line_content) = line {
                string_vec.push(line_content);
            }
            else {
                println!("Failed to read line");
            }
        }
        else if index >= max_token && index < max_token * 2 + 1 {
        ...
        //     } 
        //     else {
        //         if let Ok(num) = line_content.trim().parse::<usize>() {
        //             usize_vec.push(num);
        //         } else {
        //             // 处理解析失败的情况
        //             // println!("Failed to parse line as usize: {}", line_content);
        //         }
        //     }
        // } else {
        //     println!("Failed to read line");
...
fn count_greenlist_num(max_token_num: usize, secret_key: usize, text_token_index_list: &Vec<usize>, threshold_list: &Vec<String>) -> usize {
    ...
    for i in 0..max_token_num {
        ...
        // seed = sub_fp(seed.clone());
        let this_token_hash_inputs = [secret_key_fp, Fp::from(text_token_index_list[i] as u64), Fp::from(text_token_index_list[i + 1] as u64)];
...
fn main() {
    ...
    let (string_values, usize_values) = get_public_inputs_from_file(&cli_input_file_path, max_token_num).unwrap();
    ...
    let greenlist_count = count_greenlist_num(max_token_num, secret_key, &usize_values, &string_values);
    println!("greenlist_count: {:?}", greenlist_count);
    let z_score = compute_z_score(greenlist_count, gamma, max_token_num);
