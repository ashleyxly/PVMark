...
use halo2_merkle_tree::chips::merkle_width_32::MerkleTreeV3Circuit;
use halo2_merkle_tree::poseidon::spec_width_32::PoseidonSpec;
...
fn compute_merkle_root(leaf: &u64, elements: &Vec<u64>, indices: &Vec<u64>) -> Fp {
    let temp = WIDTH - 2;
    let remainder = elements.len() % (WIDTH - 2);
    ...
    let mut message: [Fp; WIDTH - 1] = [Fp::from(0); WIDTH - 1];
    let mut index = 0;
    for i in 0..layers {
        if i == 0 {
            ...
            message[0] = digest;
            for k in 0..temp {
                message[k + 1] = Fp::from(elements[index + k]);
            }
            index = index + temp;
            ...
            if flag == 0 {
                ...
                for k in remainder..temp {
                    message[k + 1] = Fp::from(0);
                }
                // if remainder == 1 {
...
fn compute_merkle_root_fp(leaf: &Fp, elements: &Vec<Fp>) -> Fp {
    let temp = WIDTH - 2;
    let remainder = elements.len() % (WIDTH - 2);
    let flag = if remainder == 0 {
        0
...
fn merkle_tree_poseidon_hash(leaf: Fp, elements: Vec<Fp>, proof_path: String)  {
    // let leaf = 99u64;
    // let elements = vec![1u64, 5u64, 6u64, 9u64, 9u64];
    // let indices = vec![0u64, 0u64, 0u64, 0u64, 0u64];
    let digest = compute_merkle_root_fp(&leaf, &elements);
...
fn main() {
    ...
    merkle_tree_poseidon_hash(Fp::from(leaf), model_params_fp, proof_path_str);
