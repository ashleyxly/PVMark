...
use halo2_merkle_tree::chips::summation_chip::{SummationChip, SummationConfig, SUMMATION_NUM};


#[derive(Debug, Default, Clone)]
pub struct SummationTestCircuit {
    /*Simple add test */
...
impl Circuit<Fp> for SummationTestCircuit {
    type Config = SummationConfig;
    ...
    fn configure(meta: &mut ConstraintSystem<Fp>) -> Self::Config {
        let advice = [(); SUMMATION_NUM].map(|_| meta.advice_column());
        let advice_output = meta.advice_column();
        let instance = meta.instance_column();
        
        SummationChip::configure(meta, advice, advice_output, instance)
    }

    fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<Fp>) -> Result<(), halo2_proofs::plonk::Error> {
        let chip = SummationChip::construct(config);
        ...
        // }
        /*******************/
        let mut inputs_cell_vec = vec![];
        for i in 0..self.summation_inputs.len() {
            let (summation_inputs_cell) = chip.load_private(layouter.namespace(|| format!("No. {} -- Load Summation Input", i)), self.summation_inputs[i])?;
            inputs_cell_vec.push(summation_inputs_cell);
            // let (summation_output_cell) = chip.summation(layouter.namespace(|| format!("Summation {}", i)), summation_inputs_cell)?;
            // chip.expose_public(layouter.namespace(|| format!("expose output {}", i)), &summation_output_cell, i)?;
        }

        /* test summation */
        let summation_inputs_number = self.summation_inputs.len();
        let summation_output_cell = chip.assign_multiple_value_and_summation(layouter.namespace(|| "summation"), &inputs_cell_vec, summation_inputs_number)?;
        println!("summation_output_cell: {:?}", summation_output_cell);
...
fn main() {
    ...
    let (summation_inputs, result) = generate_summation_test_case(1000);
    ...
    let circuit = SummationTestCircuit {
