...
impl Circuit<Fp> for Sha256DetectionCircuit {
    ...
    fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<Fp>) -> Result<(), halo2_proofs::plonk::Error> {
        ...
        let mut hash2_input = [hash2_input1, hash2_input2].concat();
        ...
        let mut fixed_threshold_cell = less_than_input2_cell.clone();
        ...
        for round in 0..self.text_token_value_list_length-1 {
            ...
            hash2_input = [hash2_input1, hash2_input2].concat();
...
fn generate_test_case(vocabulary_size: usize) -> (Fp, usize, Vec<usize>, usize, Fp, usize) {
    ...
    let mut less_than_flag: usize = 0;
    let mut threshold_this = threshold_temp.clone();
    if seed2 < threshold_this {
        less_than_flag = 1;
    }
    ...
    (serect_key, last_prompt_index, text_token_index_list, text_token_value_list_length, threshold_temp.clone(), count)
...
fn main() {
    ...
    public_inputs.push(text_token_hash_threshold);
    // for i in 0..text_token_value_list_length {
