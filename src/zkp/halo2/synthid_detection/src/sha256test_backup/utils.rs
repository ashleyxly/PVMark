...
pub fn process_hex_string_3(hex_string: &str) -> [u32; 8] {
    // 步骤 1: 将 hex_string 分割为 8 个长度为 8 的 string
    let chunks: Vec<&str> = hex_string.as_bytes().chunks(8).map(|chunk| std::str::from_utf8(chunk).unwrap()).collect();

    // 步骤 2: 将每个长度为 8 的 string 转化为 u32
    let u32_values: Vec<u32> = chunks.into_iter().map(|s| u32::from_str_radix(s, 16).unwrap()).collect();

    // 步骤 3: 将所有的 u32 按照倒序存到 [u32; 8]
    let mut result_array: [u32; 8] = Default::default();
    result_array.copy_from_slice(&u32_values[..8]);
    result_array.reverse();
...
pub fn convert_to_decimal_str(bytes: &[u8; 32]) -> String {
    // 将字节数组转换为大端顺序的 BigUint 数值
    let mut value = BigUint::from(0u8);
    let base = BigUint::from(256u16);

    for &byte in bytes.iter().rev() {
        value *= &base;
        value += BigUint::from_u8(byte).unwrap();
    }

    // 将 BigUint 数值转换为十进制字符串
    value.to_string()
}

pub fn convert_to_hex_str(bytes: &[u8; 32]) -> String {
    // 将字节数组转换为大端顺序的 BigUint 数值
    let mut value = BigUint::from(0u8);
    let base = BigUint::from(256u16);
...
pub fn expr_from_bytes(bytes: &[Expression<Fp>]) -> Expression<Fp> {
    ...
    let mut value = Expression::Constant(Fp::from(0u64));
    // let mut multiplier = Fp::ONE;
    let mut multiplier = Fp::from(1u64);
...
pub fn load_params_cmd_2(
    srs_path: PathBuf,
    // logrows: u32,
// ) -> ParamsKZG<Bn256> {
) -> Result<ParamsKZG<Bn256>, Box<dyn Error>> {
    let params_fs = File::open(srs_path).expect("couldn't load sha256_params");
