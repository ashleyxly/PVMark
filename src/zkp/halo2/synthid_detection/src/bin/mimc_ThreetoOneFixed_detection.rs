...
impl Circuit<Fp> for MiMCDetectionCircuit {
    ...
    fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<Fp>) -> Result<(), halo2_proofs::plonk::Error> {
        ...
        let mut hash_input2 = [Fp::from(self.secret_key), Fp::from(self.last_prompt_index as u64), Fp::from(self.text_token_index_list[0] as u64)].to_vec();
        ...
        let mut summation_mimc_inputs_3 = vec![hash_input2_cell_2.clone(), key_cell_2_2, second_element_hash_cell_2];
        let mut key_cell_2_3 = summation_mimc_chip.assign_multiple_value_and_summation(layouter.namespace(|| "Compute Final Hash_2"), &summation_mimc_inputs_3, summation_mimc_inputs_3.len())?;

        let mut hash_input2_cell_3 = layouter.assign_region(
            || "load input2_3",
            |mut region| {
                region.assign_advice(
                    || "load input2_3 message",
                    config.advice_1,
                    0,
                    || Value::known(hash_input2[2]),
                )
            }  
        )?;

        let mut third_element_hash_cell_2 = mimc7hash_chip.encrypt_message(layouter.namespace(|| "third_element_hash_2"), &hash_input2_cell_3, &key_cell_2_3)?;
        let mut final_hash_input = vec![hash_input2_cell_3.clone(), key_cell_2_3, third_element_hash_cell_2];
        let mut final_hash_cell = summation_mimc_chip.assign_multiple_value_and_summation(layouter.namespace(|| "Compute Final Hash_2"), &summation_mimc_inputs_3, summation_mimc_inputs_3.len())?;
        ...
        let mut last_prompt_index_cell = hash_input2_cell_2.clone();
        // token_index_cell_vec.push(hash_input2_cell_2.clone());
        token_index_cell_vec.push(hash_input2_cell_3.clone());
        ...
        for round in 0..self.text_token_value_list_length-1 {
            ...
            hash_input2 = [Fp::from(self.secret_key), Fp::from(self.text_token_index_list[round] as u64), Fp::from(self.text_token_index_list[round+1] as u64)].to_vec();
            ...
            summation_mimc_inputs_3 = vec![hash_input2_cell_2.clone(), key_cell_2_2, second_element_hash_cell_2];
            key_cell_2_3 = summation_mimc_chip.assign_multiple_value_and_summation(layouter.namespace(|| "Compute Final Hash_2"), &summation_mimc_inputs_3, summation_mimc_inputs_3.len())?;

            hash_input2_cell_3 = layouter.assign_region(
                || "load input2_3",
                |mut region| {
                    region.assign_advice(
                        || "load input2_3 message",
            ...
            third_element_hash_cell_2 = mimc7hash_chip.encrypt_message(layouter.namespace(|| "third_element_hash_2"), &hash_input2_cell_3, &key_cell_2_3)?;
            final_hash_input = vec![hash_input2_cell_3.clone(), key_cell_2_3, third_element_hash_cell_2];
            final_hash_cell = summation_mimc_chip.assign_multiple_value_and_summation(layouter.namespace(|| "Compute Final Hash_2"), &summation_mimc_inputs_3, summation_mimc_inputs_3.len())?;
...
fn generate_test_case(vocabulary_size: usize) -> (Fp, usize, Vec<usize>, usize, Fp, usize) {
    ...
    for _ in 0..text_token_value_list_length {
        ...
        let mut temp3 = Fp::from(7890 as u64);
        ...
        // .hash([temp, temp2]);
        let mut threshold_temp = multi_mimc7_hash_bn256_new(vec![temp, temp2, temp3]);
    ...
    let mut hash_input2 = [Fp::from(serect_key), Fp::from(last_prompt_index as u64), Fp::from(text_token_index_list[0] as u64)].to_vec();
    ...
    for i in 0..text_token_value_list_length-1 {
        ...
        hash_input2 = [Fp::from(serect_key), Fp::from(text_token_index_list[i] as u64), Fp::from(text_token_index_list[i+1] as u64)].to_vec();
