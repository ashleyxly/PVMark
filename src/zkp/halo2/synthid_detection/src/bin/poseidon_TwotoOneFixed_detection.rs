...
use halo2_detection::poseidon::spec_width_3::{POSEIDON_WIDTH, POSEIDON_RATE};
use halo2_detection::poseidon::spec_width_3::PoseidonSpec;
...
impl Circuit<Fp> for DetectTextWatermarkCircuit {
    ...
    fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<Fp>) -> Result<(), halo2_proofs::plonk::Error> {
        ...
        let test_cell = summation_chip.load_private(layouter.namespace(|| "load test"), Fp::from(0u64))?;
        summation_chip.expose_public(layouter.namespace(|| "Expose Count Result"), &test_cell, 0)?;
...
fn generate_test_case(vocabulary_size: usize, fixed_threshold: Fp) -> (Fp, usize, Vec<usize>, usize, usize) {
    ...
    let mut less_than_flag: usize = 0;
    let mut threshold_this = fixed_threshold.clone();
    if seed2 < threshold_this {
        less_than_flag = 1;
    }
    ...
    for i in 0..text_token_value_list_length-1 {
        ...
        if seed2 < fixed_threshold.clone() {
            less_than_flag = 1;
        } else {
            less_than_flag = 0;
        }
    ...
    (serect_key, last_prompt_index, text_token_index_list, text_token_value_list_length, count)
...
fn main() {
    ...
    let (serect_key, last_prompt_index, text_token_index_list, text_token_value_list_length, greenlist_count) = generate_test_case(max_token_num.clone(), text_token_hash_fixed_threshold.clone());
    ...
    public_inputs.push(Fp::from(0 as u64));
