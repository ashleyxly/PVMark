...
template detect_directly(seq_len) {
    ...
    for (var i = 0; i < seq_len; i++) {
        ...
        hasher1[i] = Poseidon(3);
        
        hasher1[i].inputs[0] <== key;
        hasher1[i].inputs[1] <== uni_gram[i];
        hasher1[i].inputs[2] <== current_token_index[i];
...
component main {public [uni_gram, current_token_index]} = detect_directly(25);
