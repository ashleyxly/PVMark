...
result_file_path="${PVMark_ZKLLM_ROOT:-external/ZKLLMWatermark_Codes}/hash_function/circom_circuit/synthid_circom/Non-Recursive-Hash/test_result/PlonK_h_${hash_type}_s_${seq_len}_w_${window_size}_d_${depth}.txt"
...
for file_full in "${circom_files[@]}"; do
    ...
    COMMAND_SETUP="snarkjs plonk setup "./${filename}.r1cs" "${PVMark_PTAU_DIR:-external/snarkjs_setup_file}/powersOfTau28_hez_final_${log2_ceil}.ptau" "${filename}_circuit_final.zkey" -v"

    # 运行命令并捕获输出
    OUTPUT=$($COMMAND_SETUP 2>&1)
    echo "$OUTPUT"
    # 检查输出中是否包含 ERROR
    if echo "$OUTPUT" | grep -q "ERROR"; then
        echo "Error encountered. Processing constraints..."


        # 提取 Plonk constraints 的值
        # CONSTRAINTS=$(echo "$OUTPUT" | grep "Plonk constraints" | grep -o -E '[0-9]+')
        # 提取 Plonk constraints 的值
        CONSTRAINTS=$(echo "$OUTPUT" | grep "Plonk constraints" | awk '{print $NF}')


        # 检查是否成功提取到了 constraints
        if [ -z "$CONSTRAINTS" ]; then
            echo "Failed to extract constraints. Exiting..."
            exit 1
        fi

        echo "Constraints found: $CONSTRAINTS"

        # 计算 log2 constraints 的 ceil 值
        LOG2_CEIL=$(python3 -c "import math; print(math.ceil(math.log2($CONSTRAINTS)))")

        echo "Calculated log2 ceil value: $LOG2_CEIL"

        log2_ceil=$LOG2_CEIL
            
        echo "Constraints: $CONSTRAINTS" >> "$result_file_path" 2>&1
        echo "new_log2_ceil: $log2_ceil" >> "$result_file_path" 2>&1

        if [ $log2_ceil -gt 24 ]; then
            echo "log2_ceil: $log2_ceil is too large, skip this file" >> "$result_file_path" 2>&1
            continue
        fi
        # 运行命令并指定 log2 constraints 的 ceil 值（假设该命令需要此参数）
        # OUTPUT=$($COMMAND_SETUP --log2 $LOG2_CEIL 2>&1)
        start=$(date +%s.%3N)
        snarkjs plonk setup "./${filename}.r1cs" "${PVMark_PTAU_DIR:-external/snarkjs_setup_file}/powersOfTau28_hez_final_${log2_ceil}.ptau" "${filename}_circuit_final.zkey" -v
    
    else
        echo "Command succeeded without errors."
    fi

    # echo "1234" | snarkjs zkey contribute "${filename}_circuit_0.zkey" "${filename}_circuit_1.zkey" --name="1st Contributor Name" -v
    snarkjs zkey export verificationkey "${filename}_circuit_final.zkey" "${filename}_verification_key.json"
    end=$(date +%s.%3N)
    runtime=$(echo "scale=3; $end - $start" | bc)
    ...
    echo "****Generate Proof****"
    start=$(date +%s.%3N)
    snarkjs plonk prove "${filename}_circuit_final.zkey" "./${filename}_cpp/${filename}_witness.wtns" "${filename}_proof.json" "${filename}_public.json"
    end=$(date +%s.%3N)
    runtime=$(echo "scale=3; $end - $start" | bc)
    ...
    echo "****Verify Proof****"
    start=$(date +%s.%3N)
    snarkjs plonk verify "${filename}_verification_key.json" "${filename}_public.json" "${filename}_proof.json" 
    end=$(date +%s.%3N)
    runtime=$(echo "scale=3; $end - $start" | bc)
