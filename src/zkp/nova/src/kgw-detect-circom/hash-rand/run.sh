...
# 解析命令行参数
while getopts "t:h:s:w:d:f:" opt; do
  case "$opt" in
...
# 确保所有必需的参数都已传入
if [ -z "$type_sampling" ] || [ -z "$hash_type" ] || [ -z "$seq_len" ] || [ -z "$window_size" ] || [ -z "$depth" ] || [ -z "$folding_num" ]; then
  echo "错误: 必须提供文件夹名、三个参数和值和文件名"
  echo "用法: $0 -t <type_sampling> -h <hash_type> -s <seq_len> -w <window_size> -d <depth> -f <folding_num>"
  exit 1
fi
...
output=$(${PVMark_NOVA_SCOTIA_ROOT:-external/Nova-Scotia}/target/release/"$hash_type"-detect -t "$type_sampling" -s "$seq_len" -w "$window_size" -d "$depth" -f "$folding_num")
...
# 创建token_num文件夹如果不存在
token_num_dir="${PVMark_NOVA_SCOTIA_ROOT:-external/Nova-Scotia}/src/kgw-detect-circom/hash-rand/results_more/${token_num}"
...
output_file="${PVMark_NOVA_SCOTIA_ROOT:-external/Nova-Scotia}/src/kgw-detect-circom/hash-rand/results_more/${token_num}/${hash_type}/${hash_type}_s_${seq_len}_w_${window_size}_d_${depth}_f_${folding_num}.txt"
