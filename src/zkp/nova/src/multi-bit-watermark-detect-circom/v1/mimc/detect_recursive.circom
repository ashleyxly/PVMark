...
template each_position_detect(partial_seq_len, each_position_infor, nLevels) {
    ...
    for (var i = 0; i < partial_seq_len; i ++)
    {
        ...
        {
            input_hasher[i][j] = MultiMiMC7(1, 91);
            input_hasher[i][j].k <== key;
            input_hasher[i][j].in[0] <== current_count[i][j];
    ...
    for (var i = 0; i < partial_seq_len; i ++)
    {
        hash_leaf[i] = MultiMiMC7(2, 91);
        hash_leaf[i].in[0] <== position[i];
        hash_leaf[i].in[1] <== current_token_index[i];
        hash_leaf[i].k <== 0;
    }
    ...
    for (var i = 0; i < partial_seq_len; i++) 
    {
        ...
        {
            hasher1[i][j] = MultiMiMC7(3, 91);
            hasher1[i][j].in[0] <== key;
            hasher1[i][j].in[1] <== ngrams[i];
            hasher1[i][j].in[2] <== j;
            hasher1[i][j].k <== 0;

            
            hasher2[i][j] = MultiMiMC7(2, 91);
            hasher2[i][j].in[0] <== hasher1[i][j].out;
            hasher2[i][j].in[1] <== current_token_index[i];
            hasher2[i][j].k <== 0;
    ...
    for (var i = 0; i < partial_seq_len; i ++)
    {
        for (var j = 0; j < each_position_infor; j ++)
        {
            ...
            output_hasher[i][j] = MultiMiMC7(1, 91);
            output_hasher[i][j].k <== key;
            output_hasher[i][j].in[0] <== output_count[i][j];
