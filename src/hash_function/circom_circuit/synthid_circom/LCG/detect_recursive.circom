...
include "../../../zkp/circom/circomlib/circuits/mimc.circom";
...
template detect_recursive(seq_len, sliding_window_size, watermarking_depth) {
    ...
    for (var i = 0; i < seq_len; i++) {
        hasher1[i] = LCG_Generator(sliding_window_size + 1);
        hasher1[i].current_hash <== 1;
        for (var j = 0; j < sliding_window_size; j++) {
            hasher1[i].data[j] <== ngrams[i][j];
        }
        hasher1[i].data[sliding_window_size] <== current_token_index[i];

        for (var k = 0; k < watermarking_depth; k ++) {
            hasher2[i][k] = LCG_Generator(1);
            hasher2[i][k].current_hash <== hasher1[i].out;
            hasher2[i][k].data[0] <== key[k];
