...
include "../../zkp/circom/circomlib/circuits/blake2/blake2b.circom";
...
template detect(T) {
    ...
    for (var i = 0; i < T; i++) {
        hasher1[i] = Blake2b_bytes(256 * 3);
        for (var j = 0; j < 256; j ++) {
            hasher1[i].inp_bytes[j] <== n2b_sk.out[j];
            hasher1[i].inp_bytes[j + 256] <== n2b_prompt_and_token_index_list[i].out[j];
            hasher1[i].inp_bytes[j + 512] <== n2b_prompt_and_token_index_list[i + 1].out[j];
        }

        b2n[i] = Bits2Num(256);
        for (var j = 0; j < 256; j++) {
            b2n[i].in[j] <== hasher1[i].hash_bits[j];
        }
