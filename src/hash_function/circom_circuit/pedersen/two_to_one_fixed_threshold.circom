...
include "../../zkp/circom/circomlib/circuits/pedersen.circom";
include "../../zkp/circom/circomlib/circuits/bitify.circom";
...
template detect(T) {
    ...
    for (var i = 0; i < T; i++) {
        hasher1[i] = Pedersen(256 * 2);
        for (var j = 0; j < 256; j++) {
            hasher1[i].in[j] <== n2b_sk.out[j];
            hasher1[i].in[j + 256] <== n2b_prompt_and_token_index_list[i].out[j];
        }
        seed[i] <== hasher1[i].out[1];

        n2b[i] = Num2Bits(256);
        n2b[i].in <== seed[i];

        hasher2[i] = Pedersen(256 * 2);
        for (var j = 0; j < 256; j++) {
            hasher2[i].in[j] <== n2b[i].out[j];
            hasher2[i].in[j + 256] <== n2b_prompt_and_token_index_list[i + 1].out[j];
        }
        hash_this[i] <== hasher2[i].out[1];
